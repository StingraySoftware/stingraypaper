...
Moreover, the functionality to plot the phaseogram, interactively change the timing parameters and adjusting the solution, and calculating the TOAs for use with external programs, is conveniently accessible in the HENDRICS tool \texttt{HENphaseogram} (See \section{sec:hendrics}).

\begin{figure*}[htbp]
\begin{center}
\includegraphics[width=\linewidth]{../figures/phaseogram.pdf}
\caption{Phaseogram showing the variation of the pulse phase corresponding to nonzero frequency derivatives in SMC X-1, executed with Stingray.}
