...
This dearth of open tools motivated the development of \stingray\footnote{\stingray was named partly in homage to the popular 1960s childrens' TV series, from where \stingray's motto derives: \textit{Anything can happen in the next half hour (but probably spectral timing)!}}, a library built entirely in \python and based on \astropy functionality, to address the lack of well-tested, well-documented software for spectral timing. 
...
The \texttt{pulse} subpackage optionally allows for just-in-time compilation using \texttt{numba} \citep{numba} for computational efficiency.
...
An example is shown in Figure \ref{fig:dynspec}.
...
We thank the Google Summer of Code Program for funding a total 5 students who implemented a large fraction of the various library components over two summers.
